...
#############################################################################################################################################
'''
#Gaussian Process Regression    #I is not working correctly
...
#print gp
print 'the kernel used to parameterize the GP, along with their corresponding optimal hyperparameter values is: ', gp.kernel_
...
accuracy = 100 - np.mean(mape)
print'Accuracy:', round(accuracy, 2), '%.'
print "R^2 for test data", gp.score(x_test,y_test)
print "R^2 for training data", gp.score(x_train,y_train)
'''
'''
...
gp.fit(pred_train, tar_train)
print 'The parameters are: ', gp.get_params(deep=True)
print 'log-marginal likelihood of theta for training data: ', gp.log_marginal_likelihood(theta=None, eval_gradient=False)


# Make the prediction on the meshed x-axis (ask for MSE as well)
y_pred, sigma = gp.predict(pred_test, return_std=True)
print 'prediction: ', y_pred
print 'sigma: ', sigma
...
accuracy = 100 - np.mean(mape)
print'Accuracy:', round(accuracy, 2), '%.'
print "R^2 for test data", gp.score(pred_test,tar_test)
print "R^2 for training data", gp.score(pred_train,tar_train)
Title='Distribution  Plot of  Predicted Value Using Test Data vs Data Distribution of Test Data'
DistributionPlot(tar_test,y_pred,"Actual Values (Test)","Predicted Values (Test)",Title)
'''
'''
...
fig = plt.figure()
plt.plot(x_test, y_test, 'r:', label='f(x) = x\,\sin(x)')
...
y_log = tar_train.values
y_log_test = tar_test.values
x_train1 = pred_train.values
x_train1 = np.atleast_2d(x_train1).T
print x_train1
x_test1= pred_test.values
...
y_log_test = y_log_test.reshape(-1,1) 
#print x_train1
...
# Calculate the absolute errors
errors = abs(y_pred - y_log_test)
# Print out the mean absolute error (mae)
print'Mean Absolute Error:', round(np.mean(errors), 2), 'degrees.'

# Calculate mean absolute percentage error (MAPE)
mape = 100 * (errors / y_log_test)
# Calculate and display accuracy
accuracy = 100 - np.mean(mape)
print'Accuracy:', round(accuracy, 2), '%.'
print "R^2 for test data", gp.score(x_test1,y_log_test)
print "R^2 for training data", gp.score(x_train1,y_log)
Title='Distribution  Plot of  Predicted Value Using Test Data vs Data Distribution of Test Data'
DistributionPlot(tar_test,y_pred,"Actual Values (Test)","Predicted Values (Test)",Title)
...
# the MSE
#fig = plt.figure()
#plt.plot(x_test1, y_log_test, 'r:', label=u'$f(x) = x\,\sin(x)$')
#plt.plot(x_train1, y_log, 'r.', markersize=10, label=u'Observations')
#plt.plot(x_test1, y_pred, 'b-', label=u'Prediction')
#plt.fill(np.concatenate([x_test1, x_test1[::-1]]),
#         np.concatenate([y_pred - 1.9600 * sigma,
#                        (y_pred + 1.9600 * sigma)[::-1]]),
#         alpha=.5, fc='b', ec='None', label='95% confidence interval')
#plt.xlabel('$x$')
...
clf = Pipeline([('scaler', StandardScaler()), ('gp', gp)])     
y_log_centered = y_log - y_log.mean()
y_pred = cross_val_predict(clf, x_train1, y_log_centered, cv=2, n_jobs=-1)
y = np.expm1(y_log)
y_pred = np.expm1(y_pred + y_log.mean())
score = rmsle(y,y_pred)
print(score) # 0.1459
...
X_train, X_test, Y_train, Y_test = train_test_split(predictors, y_data, test_size=.3, random_state=123)
...
gpr.fit(pred_train, tar_train)
y_gpr = gpr.predict(pred_test)
print y_gpr
#y_gpr_real = scaler.inverse_transform(y_gpr)
#y_train_real = scaler.inverse_transform(Y_train)
#y_test_real = scaler.inverse_transform(Y_test)

# Calculate the absolute errors
errors = abs(y_gpr - tar_test)
# Print out the mean absolute error (mae)
print'Mean Absolute Error:', round(np.mean(errors), 2), 'degrees.'
...
accuracy = 100 - np.mean(mape)
print'Accuracy:', round(accuracy, 2), '%.'
#print "R^2 for test data", gp.score(x_test1,y_test_real)
#print "R^2 for training data", gp.score(x_train1,y_train_real)
Title='Distribution  Plot of  Predicted Value Using Test Data vs Data Distribution of Test Data'
DistributionPlot(tar_test,y_gpr,"Actual Values (Test)","Predicted Values (Test)",Title)
'''